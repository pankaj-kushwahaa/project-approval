...
<div class="w3-container">
  <div class="w3-section w3-bottombar w3-padding-16">
    ...
    <a href="approved-projects.php" class="w3-button w3-white"><i class="fa-solid fa-graduation-cap mx-1"></i> All Approved<?php $sql8 = "SELECT pro_id, COUNT(pro_id) total_pro FROM project_request pr INNER JOIN branch b ON pr.pro_branch_id = b.branch_id
...
if(isset($_REQUEST['delete-approved'])){
  ...
  if($result5->execute(array($delete))){
    ...
    if($result6->execute(array(1, $branch_pro))){
     echo '<script> location.href = "'.$hostname.'/admin/approved-branch.php?branch_id='.$branch_id.'"</script>';
    }
  }
}
?>
...
<div class="w3-row pagination">
  <?php
    ...
    if($result2->rowCount()){
      ...
      echo '<ul>';
      if($page > 1){
        echo '<li class="w3-button w3-grey mx-1"><a href="approved-branch.php?branch_id='.$branch_id.'&page='.($page-1).'">Previous</a></li>';
      }
      ...
      for($i = 1; $i <= $total_pages; $i++){
        ...
        echo '<li class="w3-button w3-grey mx-1 '.$active.'"><a href="approved-branch.php?branch_id='.$branch_id.'&page='.$i.'" class="w3-blue" style="padding:100%;">'.$i.'</a></li>';
      }
      if($total_pages > $page){
        echo '<li class="w3-button w3-grey mx-1"><a href="approved-branch.php?branch_id='.$branch_id.'&page='.($page + 1).'">Next</a></li>';
      }
      echo '</ul>';
    }
  ?>

...
define('TITLE', 'All Approved Projects');
...
<div class="w3-container">
  <div class="w3-section w3-bottombar w3-padding-16">
    ...
    <a href="approved-projects.php" class="w3-button w3-black"><i class="fa-solid fa-graduation-cap mx-1"></i> All Approved<?php $sql8 = "SELECT pro_id, COUNT(pro_id) total_pro FROM project_request pr INNER JOIN branch b ON pr.pro_branch_id = b.branch_id
    ...
    ?><a href="approved-branch.php?branch_id=<?php echo $row3['branch_id'] ?>" class="w3-button w3-white"><?php echo $row3['branch_name'].' ('.$total_pro.')';; ?></a>
...
<div class="w3-container">
  ...
  <table class="w3-table w3-bordered w3-border w3-white">
    ...
    <?php
        ...
        LEFT JOIN branch b ON b.branch_id = s.stu_branch WHERE pro_approved = ?
        ORDER BY pro_id DESC LIMIT ?, ?"; 
        $result = $conn->prepare($sql);
        $result->execute(array(1, $offset, $limit));
    ...
    <tbody>
      <tr>
        ...
        <?php if(isset($Members)){echo $Members;}  ?>
          ...
          <td><?php if($_SESSION['role'] == 0){ ?>
          <form action="view-project.php" method="post" style="display:inline;"> <input type="hidden" name="project_id" value="<?php echo $row['pro_id'] ?>"><button type="submit" class="w3-btn"><i class="fa-solid fa-eye"></i></button></form><label for="modalbox" id="modal-label<?php echo $row['pro_id'] ?>" class="w3-btn"><i class='fa-solid fa-trash'></i></label>
          <!-- Custom modal box -->
...
if(isset($_REQUEST['delete-approved'])){
  ...
  if($result5->execute(array($delete))){
    ...
    if($result6->execute(array(1, $branch_pro))){
      echo '<script> location.href = "'.$hostname.'/admin/approved-projects.php"</script>';
    }
  }
}
?>
...
<div class="w3-row pagination">
  <?php
    ...
    if($result2->rowCount()){
      ...
      echo '<ul>';
      if($page > 1){
        echo '<li class="w3-button w3-grey mx-1"><a href="approved-projects.php?page='.($page-1).'">Previous</a></li>';
      }
      ...
      for($i = 1; $i <= $total_pages; $i++){
        ...
        echo '<li class="w3-button w3-grey mx-1 '.$active.'"><a href="approved-projects.php?page='.$i.'" class="w3-blue" style="padding:100%;">'.$i.'</a></li>';
      }
      if($total_pages > $page){
        echo '<li class="w3-button w3-grey mx-1"><a href="approved-projects.php?page='.($page + 1).'">Next</a></li>';
      }
      echo '</ul>';
    }
  ?>

...
<div class="w3-container">
  <div class="w3-section w3-bottombar w3-padding-16">
    ...
    <a href="disapproved-projects.php" class="w3-button w3-white"><i class="fa-solid fa-graduation-cap mx-1"></i> All Disapproved<?php $sql8 = "SELECT pro_id, COUNT(pro_id) total_pro FROM project_request pr INNER JOIN branch b ON pr.pro_branch_id = b.branch_id
    ...
    ?><a href="disapproved-branch.php?branch_id=<?php echo $row3['branch_id'] ?>" class="w3-button w3-white <?php echo $black; ?>"><?php echo $row3['branch_name'].' ('.$total_pro.')';; ?></a>
    
    <!-- <a href="" class="w3-button w3-white"><i class="fa fa-photo w3-margin-right"></i>EE 6th</a> -->
...
<div class="w3-container"><?php 
  ...
  <table class="w3-table w3-bordered w3-border w3-white">
    ...
    <?php
        ...
        WHERE pro_branch_id = ? AND pr.pro_approved = ? AND pr.pro_review = ?
        ORDER BY pro_id DESC LIMIT ?, ?"; 
        $result = $conn->prepare($sql);
        $result->execute(array($branch_id, 0, 1, $offset, $limit));
...
if(isset($_REQUEST['delete-approved'])){
  ...
  if($result5->execute(array($delete))){
    ...
    if($result6->execute(array(1, $branch_pro))){
      echo '<script> location.href = "'.$hostname.'/admin/disapproved-branch.php?branch_id='.$branch_id.'"</script>';
    }
  }
}
?>
...
<div class="w3-row pagination">
  <?php
    ...
    INNER JOIN student s ON pr.pro_stu_id = s.stu_id
    WHERE pr.pro_branch_id = ? AND pr.pro_approved = ? ANd pr.pro_review = ?";

    $result2 = $conn->prepare($sql2);
    $result2->execute(array($branch_id, 0, 1));
    ...
    if($result2->rowCount()){
      ...
      echo '<ul>';
      if($page > 1){
        echo '<li class="w3-button w3-grey mx-1"><a href="disapproved-branch.php?branch_id='.$branch_id.'&page='.($page-1).'">Previous</a></li>';
      }
      ...
      for($i = 1; $i <= $total_pages; $i++){
        ...
        echo '<li class="w3-button w3-grey mx-1 '.$active.'"><a href="disapproved-branch.php?branch_id='.$branch_id.'&page='.$i.'" class="w3-blue" style="padding:100%;">'.$i.'</a></li>';
      }
      if($total_pages > $page){
        echo '<li class="w3-button w3-grey mx-1"><a href="disapproved-branch.php?branch_id='.$branch_id.'&page='.($page + 1).'">Next</a></li>';
      }
      echo '</ul>';
    }
  ?>

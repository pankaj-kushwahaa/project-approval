...
if(isset($_REQUEST['del-sta-btn'])){
  ...
  if($result5->execute(array($sta_id))){
...
<div class="w3-container">
  <div class="w3-section w3-bottombar w3-padding-16">
    ...
    <a href="project-status.php" class="w3-button w3-black">All Projects<?php $sql8 = "SELECT sta_id, COUNT(sta_id) total_pro FROM project_status ps INNER JOIN branch b ON ps.sta_branch_id = b.branch_id";
    ...
    ?>
    <a href="status-branch.php?branch_id=<?php echo $row3['branch_id'] ?>"  class="w3-button w3-white"><?php echo $row3['branch_name'].' ('.$total_pro.')'; ?></a>
    ...
    <tbody>
      <tr>
        ...
        <?php if(isset($Members)){echo $Members;}  ?>
         ...
         <td><form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post"><input type="hidden" name="del-status" value="<?php if(isset($row['stu_id'])){ echo $row['sta_id']; }  ?>"><input type="submit" value="Delete" name="del-sta-btn" class="w3-btn w3-green"></form>
         <?php $sql9 = "SELECT pro_id FROM project_request WHERE pro_stu_id = ? AND pro_approved = ?";
...
<div class="w3-row pagination">
  <?php
    ...
    INNER JOIN student s ON ps.sta_stu_id = s.stu_id";
    ...
    if($result2->rowCount()){
      ...
      echo '<ul>';
      if($page > 1){
        echo '<li class="w3-button w3-grey mx-1"><a href="project-status.php?page='.($page-1).'">Previous</a></li>';
      }
      ...
      for($i = 1; $i <= $total_pages; $i++){
        ...
        echo '<li class="w3-button w3-grey mx-1 '.$active.'"><a href="project-status.php?page='.$i.'" class="w3-blue" style="padding:100%;">'.$i.'</a></li>';
      }
      if($total_pages > $page){
        echo '<li class="w3-button w3-grey mx-1"><a href="project-status.php?page='.($page + 1).'">Next</a></li>';
      }
      echo '</ul>';
    }
  ?>

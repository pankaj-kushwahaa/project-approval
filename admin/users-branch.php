...
<div class="w3-container">
  <div class="w3-section w3-bottombar w3-padding-16">
    ...
    ?>
    <a href="users-branch.php?branch_id=<?php echo $row3['branch_id'] ?>"  class="w3-button w3-white <?php echo $black; ?>"><?php echo $row3['branch_name'].' ('.$total_pro.')'; ?></a>
...
<div class="w3-container">
  ...
  $result7 = $conn->prepare($sql7);
  $result7->execute(array($branch_id));
  if($result7->rowCount()){
    $row7 = $result7->fetch(PDO::FETCH_ASSOC);
  }
  ?>
  <h5>All <?php echo $row7['branch_name']; ?> Users</h5>
    <!-- Table Start -->
    <!-- Table Start -->
...
<?php
        $sql = "SELECT stu_id, stu_branch, stu_email, stu_phone, stu_password, stu_name, stu_rollno, branch_name FROM student s
        LEFT JOIN branch b ON s.stu_branch = b.branch_id WHERE s.stu_branch = ?
...
<div class="w3-row pagination">
  <?php
    $sql2 = "SELECT stu_id FROM student WHERE stu_branch = ?";
    ...
    if($result2->rowCount()){
      ...
      echo '<ul>';
      if($page > 1){
        echo '<li class="w3-button w3-grey mx-1"><a href="users-branch.php?branch_id='.$branch_id.'&page='.($page-1).'">Previous</a></li>';
      }
      ...
      for($i = 1; $i <= $total_pages; $i++){
        ...
        echo '<li class="w3-button w3-grey mx-1 '.$active.'"><a href="users-branch.php?branch_id='.$branch_id.'&page='.$i.'" class="w3-blue" style="padding:100%;">'.$i.'</a></li>';
      }
      if($total_pages > $page){
        echo '<li class="w3-button w3-grey mx-1"><a href="users-branch.php?branch_id='.$branch_id.'&page='.($page + 1).'">Next</a></li>';
      }
      echo '</ul>';
    }
  ?>
